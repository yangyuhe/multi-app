...
function App() {
  const send = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://10.252.26.28:8080/api/v1/completions", {
        method: "POST",
        body: JSON.stringify({
          ...JSON.parse(extraBody),
          prompt,
        }),
        headers: {
  ...
  >(JSON.parse(localStorage.getItem("test-ai-options") || "[]"));
  const save = () => {
    ...
    localStorage.setItem("test-ai-options", JSON.stringify(newOptions));
